<div class="master-list-grid">
    @php
        // Moved outside loop to prevent N+1 queries
                ...
                <div class="master-card-left">
                    ...
                    <div class="master-info">
                        ...
                        <span class="master-sub"
                            style="font-size: 0.85rem; opacity: 0.8; font-weight: 400;">{{ $employee->position }} •
                            {{ $employee->agency }}</span>
                    </div>
                </div>

                <div class="master-card-right">
                    @if($canEditMasterlist)
...
<div class="pagination-footer"
    style="margin-top: 2rem; display: flex; justify-content: space-between; align-items: center; background: var(--bg-card); padding: 1rem 1.5rem; border-radius: 20px; border: 1px solid var(--border); box-shadow: 0 10px 30px -10px rgba(0,0,0,0.1);">
    <div class="pagination-info" style="font-size: 0.85rem; color: var(--text-muted); font-weight: 600;">
        Showing <span style="color: var(--primary);">{{ $employees->firstItem() ?? 0 }}</span> - <span
            style="color: var(--primary);">{{ $employees->lastItem() ?? 0 }}</span> of <span
            style="color: var(--text-main);">{{ $employees->total() }}</span>
    </div>
    ...
    <div class="pagination-actions" style="display: flex; gap: 0.75rem; align-items: center;">
        ...
        @else
            <a href="javascript:void(0)" class="btn-nav btn-nav-active pagination-ajax"
                data-page="{{ $employees->currentPage() - 1 }}" title="Previous Page">
                <i data-lucide="chevron-left"></i>
            </a>
        @endif

        <div
            style="display: flex; align-items: center; gap: 0.6rem; font-weight: 700; color: var(--text-muted); font-size: 0.85rem; background: var(--bg-main); padding: 0.25rem 0.75rem; border-radius: 12px; border: 1px solid var(--border);">
            <input type="number" id="pageNumberInput" value="{{ $employees->currentPage() }}" min="1"
                max="{{ $employees->lastPage() }}"
        ...
        @if ($employees->hasMorePages())
            <a href="javascript:void(0)" class="btn-nav btn-nav-active pagination-ajax"
                data-page="{{ $employees->currentPage() + 1 }}" title="Next Page">
